...
| TURNS (TLS) | ANY | ANY | `turn.telnyx.com` / TURN server IPs | **443** |
...
| TURNS over TLS | `turns:turn.telnyx.com:443` |
...
3. If TCP port 3478 is also blocked, allow TURNS (TURN over TLS) on port **443** via `turns:turn.telnyx.com:443`. Ensure TCP port 443 is open to the TURN server IPs.

...
| `47001` | `ICE_RESTART_FAILED`                 | ICE restart failed                                       | ICE restart Modify request failed or was rejected                                           | Fatal for call    | Call may recover via WebSocket reconnect + Attach; otherwise retry the call |
| `47002` | `ONLY_HOST_ICE_CANDIDATES_EXHAUSTED` | Call terminated: only local network candidates available | STUN/TURN unreachable across multiple ICE gathering attempts; only host candidates gathered | Fatal for call | Verify STUN/TURN config; check firewall/VPN; try TCP-based TURN; set `allowCallWithHostCandidatesOnly: true` to allow host-only calls (exhausted error still fires at threshold) |

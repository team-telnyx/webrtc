...
| `47002` | `ONLY_HOST_ICE_CANDIDATES_EXHAUSTED` | Call terminated: only local network candidates available | STUN/TURN unreachable across multiple ICE gathering attempts; only host candidates gathered | Fatal for call    | Verify STUN/TURN config; check firewall/VPN; try TCP-based TURN             |

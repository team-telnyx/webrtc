...
describe('Messages', function () {
  describe('BladeConnect', function () {
    ...
    it('should match struct without sessionId', function () {
      ...
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.connect","params":{"authentication":{"project":"project","token":"token"},"version":{"major":2,"minor":1,"revision":0}}}`
      );
    ...
    it('should match struct with sessionId', function () {
      ...
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.connect","params":{"authentication":{"project":"project","token":"token"},"version":{"major":2,"minor":1,"revision":0},"sessionid":"${sessionId}"}}`
      );
      expect(message).toEqual(res);
    });
    it('should match struct with jwt_token', function () {
      ...
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.connect","params":{"authentication":{"project":"project","jwt_token":"token"},"version":{"major":2,"minor":1,"revision":0}}}`
      );
      expect(message).toEqual(res);
    });
    it('should match struct with agent', function () {
      ...
      const message = new Connect(auth).request;
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.connect","params":{"authentication":{"project":"project","token":"token"},"version":{"major":2,"minor":1,"revision":0},"agent":"Jest SDK/1.0.0"}}`
      );
      expect(message).toEqual(res);
    });
  });
  ...
  describe('BladeExecute', function () {
    it('Request should match struct', function () {
      ...
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.execute","params":{"requester_nodeid":"S1","responder_nodeid":"S2","protocol":"myprot","method":"sayHello","params":{"name":"Joe"}}}`
      );
      expect(message).toEqual(res);
    });
    it('Response should match struct', function () {
      ...
      const message = new Execute(params).request;
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","result":{"requester_nodeid":"S1","responder_nodeid":"S2","protocol":"myprot","result":{"msg":"Hello Joe!"}}}`
      );
      expect(message).toEqual(res);
    });
  });
  ...
  describe('BladeSubscription', function () {
    describe('Add', function () {
      it('should match struct', function () {
        ...
        const res = JSON.parse(
          `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.subscription","params":{"command":"add","protocol":"myprotocol","channels":["myprotocol.channel"]}}`
        );
        expect(message).toEqual(res);
      });
    });
  ...
  describe('BladePing', () => {
    it('should match the struct', () => {
      ...
      const res = JSON.parse(
        `{"jsonrpc":"2.0","id":"${message.id}","method":"blade.ping","params":{}}`
      );
      expect(message).toEqual(res);
    });
  });

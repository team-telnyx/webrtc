...
describe('active-calls recovery marker storage', () => {
  ...
  // ── VSDK-467: forceRelayCandidate boolean round-trip and matching ──
  it('round-trips the forceRelayCandidate boolean through the marker (VSDK-467)', () => {
    const now = Date.now();
    setActiveCallsRecoveryMarker(
      [
        asStoredCall({
    ...
    const relayTrue = result!.calls.find((c) => c.id === 'call-relay-true');
    expect(relayTrue).toBeDefined();
    expect(relayTrue!.forceRelayCandidate).toBe(true);

    const relayAbsent = result!.calls.find((c) => c.id === 'call-relay-absent');
    expect(relayAbsent).toBeDefined();
    // Absence is backward compatible (legacy markers).
    expect(relayAbsent!.forceRelayCandidate).toBeUndefined();

    clearActiveCallsRecoveryMarker();
  });

  it('returns the forceRelayCandidate value verbatim (no boolean coercion) when the top-level shape is valid (VSDK-467)', () => {
    // The marker helper does not per-record filter; it returns the parsed
    // payload as-is. Malformed non-boolean values are returned verbatim and
    // the CONSUMER (VertoHandler) is responsible for checking `=== true`.
    // This pins that contract: a truthy-but-not-boolean value is NOT coerced
    // to true by the storage layer.
    const now = Date.now();
    sessionStorage.setItem(
      ACTIVE_CALLS_KEY,
      JSON.stringify({
        sessionId: 'sess-malformed-relay',
        storedAt: now,
        calls: [
          { id: 'c-true', forceRelayCandidate: true },
          { id: 'c-string', forceRelayCandidate: 'yes' },
          { id: 'c-number', forceRelayCandidate: 1 },
          { id: 'c-absent' },
        ],
      })
    );
    ...
    expect(result!.calls[0].forceRelayCandidate).toBe(true);
    // Non-boolean values are returned verbatim (no coercion).
    expect(result!.calls[1].forceRelayCandidate).toBe('yes');
    expect(result!.calls[2].forceRelayCandidate).toBe(1);
    expect(result!.calls[3].forceRelayCandidate).toBeUndefined();

...
describe('CantinaAuth', () => {
  ...
  describe('userLogin', () => {
    it('should expose userLogin to get jwt for a user', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${auth.baseUrl}/login/user`, {
        ...
        body:
          '{"username":"username","password":"password","hostname":"jest.relay.com"}',
      });
    });
    ...
    it('should return the error if fetch failed', async () => {
      ...
      expect.assertions(2);
      await expect(auth.userLogin('username', 'password')).rejects.toEqual(
        expect.any(Error)
      );
      expect(global.fetch).toHaveBeenCalledTimes(1);
    });
  ...
  describe('guestLogin', () => {
    it('should expose guestLogin to get jwt for a guest', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(`${auth.baseUrl}/login/guest`, {
        ...DEFAULT_FETCH_OPTIONS,
        body:
          '{"name":"name","email":"email","token":"uuid","hostname":"jest.relay.com"}',
      });
    });
    ...
    it('should return the error if fetch failed', async () => {
      ...
      expect.assertions(2);
      await expect(auth.guestLogin('name', 'email', 'uuid')).rejects.toEqual(
        expect.any(Error)
      );
      expect(global.fetch).toHaveBeenCalledTimes(1);
    });
  ...
  describe('checkInviteToken', () => {
    ...
    it('should return the error if fetch failed', async () => {
      ...
      expect.assertions(2);
      await expect(auth.checkInviteToken('uuid')).rejects.toEqual(
        expect.any(Error)
      );
      expect(global.fetch).toHaveBeenCalledTimes(1);
    });

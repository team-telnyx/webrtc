...
const isUnifiedPlan = (sdp) => {
  ...
  if (
    parsedSDP.media.length <= 3 &&
    parsedSDP.media.every(
      (m) => ['video', 'audio', 'data'].indexOf(m.mid) !== -1
    )
  ) {
    logger.info(
...
const toPlanB = (unifiedPlanSDP) => {
  ...
  logger.info(
    ...
    `Converted Unified Plan to Plan B ${JSON.stringify(planBSDP)}`
  );
  return planBSDP;
};
...
const toUnifiedPlan = (planBSDP) => {
  ...
  logger.info(
    ...
    `Converted Plan B to Unified Plan ${JSON.stringify(unifiedPlanSDP)}`
  );

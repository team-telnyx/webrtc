...
export default (klass: any) => {
  ...
  describe('VertoHandler', () => {
    ...
    beforeEach(() => {
      instance = new klass({ host: 'example.telnyx.com', login: 'login', password: 'password', project: 'project', token: 'token' })
    ...
    describe('telnyx_rtc.invite', () => {
      it('should create a new Call in ringing state', async done => {
        ...
        const msg = JSON.parse(`{"jsonrpc":"2.0","id":4402,"method":"telnyx_rtc.invite","params":{"callID":"${callId}","sdp":"SDP","caller_id_name":"Extension 1004","caller_id_number":"1004","callee_id_name":"Outbound Call","callee_id_number":"1003","display_direction":"outbound"}}`)
        handler.handleMessage(msg)
    ...
    describe('with an active outbound Call', () => {
      ...
      describe('telnyx_rtc.answer', () => {
        it('should pass the msg to the call and reply back to the server', () => {
          const msg = JSON.parse('{"jsonrpc":"2.0","id":4404,"method":"telnyx_rtc.answer","params":{"callID":"e2fda6dc-fc9d-4d77-8096-53bb502443b6"}}')
          handler.handleMessage(msg)
          expect(call.handleMessage).toBeCalledTimes(1)
          expect(Connection.mockSend).toHaveBeenLastCalledWith({ request: { jsonrpc: '2.0', id: 4404, result: { method: 'telnyx_rtc.answer' } } })
        })
      })
    })
    ...
    describe('telnyx_rtc.info', () => {
      it('should dispatch a notification', () => {
        handler.handleMessage(JSON.parse('{"jsonrpc":"2.0","id":37,"method":"telnyx_rtc.info","params":{"fake":"data", "test": "data"}}'))
